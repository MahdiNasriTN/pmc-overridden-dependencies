...
abstract class IRowState {
  ...
  void setPendingRowColor(PlutoRow row, Color? color);
}
...
mixin RowState implements IPlutoGridState {
  ...
  @override
  List<PlutoRow> get checkedRows => refRows.where((row) => row.checked ?? false).toList(
    growable: false,
  );
  ...
  final Map<Key, Color?> _storedColors = {};
  Color? _lastUsedColor;
  ...
  void setRowChecked(
    ...
    // When checking, store current color if any
    if (flag) {
      for (var cell in findRow.cells.values) {
        if (cell.overridedColor != null) {
          _storedColors[cell.key] = cell.overridedColor!;
          cell.overridedColor = null;
        }
      }
    }
    // When unchecking, restore stored color if any
    else {
      for (var cell in findRow.cells.values) {
        if (_storedColors.containsKey(cell.key)) {
          cell.overridedColor = _storedColors[cell.key];
          _storedColors.remove(cell.key);
        }
      }
    }
  ...
  void changeColor(PlutoRow row, Color? color) {
    final findRow = refRows.firstWhereOrNull(
          (element) => element.key == row.key,
    );

    if (findRow == null) return;

    _lastUsedColor = color;

    // If row is checked, store color for later
    if (findRow.checked == true) {
      if (color != null) {
        _storedColors[findRow.key] = color;
      } else {
        _storedColors.remove(findRow.key);
      }
    } else {
      // Apply color directly to all cells
      for (var cell in findRow.cells.values) {
        cell.overridedColor = color;
      }
    }

    notifyListeners(true, changeColor.hashCode);
  }
  ...
  void setPendingRowColor(PlutoRow row, Color? color) {
    ...
    // If row is checked, uncheck it first
    if (findRow.checked == true) {
      setRowChecked(findRow, false, notify: false);
    }
    ...
    notifyListeners(true, setPendingRowColor.hashCode);
  }

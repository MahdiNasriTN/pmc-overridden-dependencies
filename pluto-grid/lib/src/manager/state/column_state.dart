...
abstract class IColumnState {
  ...
  void hideColumn(PlutoColumn column,
      bool hide, {
        bool notify = true,
      });
  ...
  /// [column.frozen] is changed to [PlutoColumnFrozen.none]
  /// if the frozen column width constraint is narrow.
  void hideColumns(List<PlutoColumn> columns,
      bool hide, {
        bool notify = true,
      });
  ...
  bool limitHideColumn(PlutoColumn column,
      bool hide, {
...
mixin ColumnState implements IPlutoGridState {
  ...
  void autoFitColumn(BuildContext context, PlutoColumn column) {
    String maxValue = '';
    double padding = 45;
    if (column.hasBestFitByHeader || column.field == 'checkbox_field') {
      maxValue = column.title;
    } else {
      padding = 60;
      maxValue = refRows.fold('', (previousValue, element) {
    ...
    // Create a TextSpan with the determined maximum value and style
    TextSpan textSpan = TextSpan(
      style: textStyle,
      text: maxValue,
    );

    // Create a TextPainter to measure the width
    ...
    // Calculate the width including some padding/margin
    double calculatedWidth = textPainter.width + padding;
  ...
  @override
  void autoFitColumnByHeaderAndData(BuildContext context, PlutoColumn column, {bool isHeaderFit = false}) {
    // Determine the maximum width value
    String maxValue = '';
    double padding = 45;
    if (isHeaderFit) {
      maxValue = column.title;
  ...
  void sortAscending(PlutoColumn column, {bool notify = true}) {
    ...
    DateTime? date;
    try {
      date = dateFormat.parseLoose(refRows
          .map((element) => element.cells[column.field]!.valueForSorting)
          .first);
    } catch (_) {
      date = null;
    }
    ...
    if (date != null) {
      ...
      refRows.sort((a, b) {
        try {
          return dateFormat.parseLoose(a.cells[column.field]!.valueForSorting)!
      ...
      // Other types sorting
      int compare(a, b) {
        var valueA = a.cells[column.field]!.valueForSorting;
        var valueB = b.cells[column.field]!.valueForSorting;
        ...
        // Try parsing as double values
        double? doubleA;
        double? doubleB;
        try {
          doubleA = double.parse(valueA.toString().replaceAll(' ', ''));
          doubleB = double.parse(valueB.toString().replaceAll(' ', ''));
        } catch (_) {
          doubleA = null;
          doubleB = null;
        }
  ...
  void sortDescending(PlutoColumn column, {bool notify = true}) {
    ...
    if (date != null) {
      ...
      refRows.sort((a, b) {
        try {
          return dateFormat.parseLoose(b.cells[column.field]!.valueForSorting)!
      ...
      int compare(a, b) {
        ...
        // Fallback comparison for other types
        return column.type.compare(valueB, valueA);
      }
  ...
  bool _limitFrozenColumn(PlutoColumnFrozen frozen,
      double offsetWidth,) {
    if (frozen.isNone) {
      return false;
    }

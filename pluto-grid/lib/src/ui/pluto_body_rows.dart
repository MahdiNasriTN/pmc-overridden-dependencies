...
class PlutoBodyRowsState extends PlutoStateWithChange<PlutoBodyRows> {
  ...
  List<PlutoColumn> _getColumns() {
    final columns = stateManager.showFrozenColumn == true ? stateManager.bodyColumns : stateManager.columns;
      ...
      child: SingleChildScrollView(
        ...
        child: CustomSingleChildLayout(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (ctx, i) {
              if (_rows[i].heightWidget != null) {
                return SizedBox(
                  height: (_rows[i].isShow ? stateManager.rowTotalHeight : 0) + _rows[i].heightWidget!,
                  child: Column(
                    children: [
                      InkWell(
                          onTap: () => stateManager.toggleExpandedRowGroup(
                                rowGroup: _rows[i],
                              ),
                          child: stateManager.isExpandedGroupedRow(
                            _rows[i],
                          )
                              ? _rows[i].widgetRowOpened ?? const SizedBox()
                              : _rows[i].widgetRowClosed ?? const SizedBox()),
                      if (_rows[i].isShow)
                        PlutoBaseRow(

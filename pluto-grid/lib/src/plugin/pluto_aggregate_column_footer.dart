...
class PlutoAggregateColumnFooterState extends PlutoStateWithChange<PlutoAggregateColumnFooter> {
  ...
  Widget build(BuildContext context) {
    ...
    column.footerValue = formattedValue;
    if (widget.isInte) {
      column.footerValueInt = formattedValue;
    }
    if (widget.isExte) {
      column.footerValueExt = formattedValue;
    }
    final children = hasTitleSpan ? widget.titleSpanBuilder!(formattedValue) : null;
    return GestureDetector(
      onSecondaryTapDown: (details) {
        _showPopupMenu(context, details.globalPosition, formattedValue);
      },
      child: Padding(
  ...
  void _showPopupMenu(BuildContext context, Offset globalPosition, String text) async {
    final RenderBox? overlayBox = Overlay
        .of(context)
        ?.context
        .findRenderObject() as RenderBox?;
    // Check if overlayBox is not null before using it
    if (overlayBox != null) {
      ...
      final PopupMenuEntry<int> copiedOption = PopupMenuItem<int>(
        ...
        Row(
          children: [
            ...
            Icon(Icons.copy, size: 15, color: Color(0xFF023C6F),),
            const SizedBox(
              width: 5,
            ),
      ...
      final int? selection = await showMenu(
      ...
      // Check if selection is not null before using it
      if (selection != null && selection == 0) {
        _copyToClipboard(text.replaceAll("\$", '')); // Call the function to copy to clipboard
      }
    }
  }

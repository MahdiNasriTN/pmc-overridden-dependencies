...
import '../ui/columns/pluto_column_filter.dart';
...
class PlutoColumn {
  ...
  String field;
  final GlobalKey<PlutoColumnFilterState>? filterKey;

  String? footerValue;
  String? footerValueInt;
  String? footerValueExt;
  ...
  bool hasOnlySort;
  bool withHideColumn;
  //VoidCallback? applyFilterCallback;
  ...
  PlutoColumn({
    ...
    this.hasOnlySort = false,
    this.withHideColumn = true,
    this.disabledFilter,
    this.titleColor,
    this.renderer,
    this.footerRenderer,
    this.hasBestFitByHeader = false ,
  ...
  String limitTextByMaxLength(String text) {
    if (text.length > maxLength) {
      return text.substring(0, maxLength);
    }
    return text;
  }
  dynamic enforceMaxLengthLimit(dynamic value) {
  ...
  void applyCellValueLimit(PlutoCell cell) {
    if (cell.value is String) {
      cell.value = limitTextByMaxLength(cell.value.toString());
    }
  }
  ...
  String formattedValueForType(dynamic value) {
    ...
    value = enforceMaxLengthLimit(value);

    if (type is PlutoColumnTypeWithNumberFormat) {
      formattedValue = type.applyFormat(value);
    } else {
  ...
  String formattedValueForDisplay(dynamic value) {
    ...
    if (formatter != null) {
      formattedValue = formatter!(value).toString();
    } else {
      formattedValue = formattedValueForType(value);
    }

    return limitTextByMaxLength(formattedValue);
  }
  ...
  String formattedValueForDisplayInEditing(dynamic value) {
    ...
    if (type is PlutoColumnTypeWithNumberFormat) {
      formattedValue = value.toString().replaceFirst(
        '.',

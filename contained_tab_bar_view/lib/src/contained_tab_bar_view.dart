      ...
      Positioned.fill(
        child: Padding(
          ...
          child: Builder(builder: (context) {
            // If the user didn't supply a Decoration `indicator` but provided an
            // `indicatorColor`, construct a consistent UnderlineTabIndicator
            // using the configured weight and padding. This gives predictable
            // results across different consumers of the package.
            final Decoration? effectiveIndicator = widget.tabBarProperties.indicator ??
                (widget.tabBarProperties.indicatorColor != null
                    ? UnderlineTabIndicator(
                        borderSide: BorderSide(
                          width: widget.tabBarProperties.indicatorWeight,
                          color: widget.tabBarProperties.indicatorColor!,
                        ),
                        insets: widget.tabBarProperties.indicatorPadding,
                      )
                    : null);

            // Default to TabBarIndicatorSize.tab for consistency unless caller
            // explicitly provided a different size.
            final TabBarIndicatorSize effectiveIndicatorSize = widget.tabBarProperties.indicatorSize ?? TabBarIndicatorSize.tab;

            return TabBar(
              controller: _controller,
              tabs: widget.tabs,
              indicator: effectiveIndicator,
              // keep indicatorPadding available for callers who set it

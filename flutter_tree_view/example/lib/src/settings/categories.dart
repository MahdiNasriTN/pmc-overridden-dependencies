...
class DarkModeButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IconButton(
      icon: icon,
      tooltip: tooltip,
      onPressed: () => context.read<SettingsController>().updateBrightness(oppositeBrightness),
    );
  }
}
    ...
    return ExpansionTile(
      ...
      children: [
        GridView.extent(
          ...
          children: const <Color>[Colors.black, Colors.white].followedBy(Colors.primaries.reversed).followedBy(Colors.accents).map(ColorOption.fromColor).toList(growable: false),
        ),
      ],
    );
  }
...
class Indent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _SliderListTile(
      ...
      min: 0.0,
      max: 64.0,
      onChanged: (value) => context.read<SettingsController>().updateIndent(value.roundToDouble()),
    );
  }
}
    ...
    return ListTile(
      ...
      onTap: () async {
        ...
        final LineStyle? newLineStyle = await showMenu<LineStyle>(
          context: context,
          position: RelativeRect.fromLTRB(0, 0, tile.size.width, 0),
...
class Direction extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      onTap: () => context.read<SettingsController>().updateTextDirection(oppositeTextDirection),

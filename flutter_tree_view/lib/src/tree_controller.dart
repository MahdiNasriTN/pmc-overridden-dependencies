...
class TreeController<T extends Object> with ChangeNotifier {
  ...
  TreeSearchResult<T> search(ValuePredicate<T> predicate) {
    ...
    Tuple2<int, int> traverse(Iterable<T> nodes) {
      ...
      for (final T child in nodes) {
        ...
        final Tuple2<int, int> result = traverse(childrenProvider(child));
        totalNodeCount += result.item1 + 1;
        totalMatchCount += result.item2;

        if (result.item2 > 0) {
          allMatches[child] = TreeSearchMatch(
            isDirectMatch: allMatches[child]?.isDirectMatch ?? false,
            subtreeNodeCount: result.item1,
            subtreeMatchCount: result.item2,
          );
        }
      }
      return Tuple2(totalNodeCount, totalMatchCount);
    }

    final Tuple2<int, int> result = traverse(roots);
    return TreeSearchResult<T>(
      matches: allMatches,
      totalNodeCount: result.item1,
      totalMatchCount: result.item2,
    );
  }
...
class TreeSearchMatch with Diagnosticable {
  ...
  bool operator ==(Object other) {
    ...
    return other is TreeSearchMatch && other.runtimeType == runtimeType && other.isDirectMatch == isDirectMatch && other.subtreeNodeCount == subtreeNodeCount && other.subtreeMatchCount == subtreeMatchCount;
  }
}

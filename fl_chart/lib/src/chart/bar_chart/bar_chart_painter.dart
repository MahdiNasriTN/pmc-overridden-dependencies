...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  void paint(BuildContext context, CanvasWrapper canvasWrapper, PaintHolder<BarChartData> holder) {
    ...
    final groupsX = calculateGroupsX(canvasWrapper.size, data.barGroups, data.alignment, holder);
    _groupBarsPosition = calculateGroupAndBarsPosition(canvasWrapper.size, groupsX, data.barGroups);
  ...
  @visibleForTesting
  List<double> calculateGroupsX(Size viewSize, List<BarChartGroupData> barGroups, BarChartAlignment alignment, PaintHolder<BarChartData> holder) {
    final data = holder.data;
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        if (barRod.backDrawRodData.show && barRod.backDrawRodData.y != 0) {
          if (barRod.backDrawRodData.y > 0) {
            ...
            final top = min(getPixelY(barRod.backDrawRodData.y, drawSize, holder), bottom - cornerHeight);
            ...
            final bottom = max(getPixelY(barRod.backDrawRodData.y, drawSize, holder), top + cornerHeight);
        ...
        if (barRod.y != 0) {
          ...
          if (barRod.rodStackItems.isNotEmpty) {
            for (var i = 0; i < barRod.rodStackItems.length; i++) {
              ...
              drawStackItemBorderStroke(canvasWrapper, stackItem, i, barRod.rodStackItems.length, barRod.width, barRRect, drawSize, holder);
            }
          }
        }
      }
  ...
  void drawTitles(
    ...
    final leftInterval = leftTitles.interval ?? Utils().getEfficientInterval(viewSize.height, data.verticalDiff);
    if (leftTitles.showTitles) {
      AxisChartHelper().iterateThroughAxis(
        ...
        action: (axisValue) {
          if (leftTitles.checkToShowTitle(data.minY, data.maxY, leftTitles, leftInterval, axisValue)) {
            var x = 0 + getLeftOffsetDrawSize(holder);
            var y = getPixelY(axisValue, drawSize, holder);

            final text = leftTitles.getTitles(axisValue);

            final span = TextSpan(
              style: Utils().getThemeAwareTextStyle(context, leftTitles.getTextStyles(context, axisValue)),
              text: text,
            );
            ...
            y -= tp.height / 2;
            x += Utils().calculateRotationOffset(tp.size, leftTitles.rotateAngle).dx;
    ...
    if (topTitles.showTitles && groupBarsPosition.containsAnythingToShow()) {
      for (var index = 0; index < groupBarsPosition.length; index++) {
        ...
        final span = TextSpan(
          style: Utils().getThemeAwareTextStyle(context, topTitles.getTextStyles(context, xValue)),
          text: text,
        );
        final tp = TextPainter(
          text: span,
    ...
    final rightInterval = rightTitles.interval ?? Utils().getEfficientInterval(viewSize.height, data.verticalDiff);
    if (rightTitles.showTitles) {
      AxisChartHelper().iterateThroughAxis(
        ...
        action: (axisValue) {
          if (rightTitles.checkToShowTitle(data.minY, data.maxY, rightTitles, rightInterval, axisValue)) {
            ...
            final span = TextSpan(
              style: Utils().getThemeAwareTextStyle(context, rightTitles.getTextStyles(context, axisValue)),
              text: text,
            );
            final tp = TextPainter(
              text: span,
            ...
            y -= tp.height / 2;
            x -= Utils().calculateRotationOffset(tp.size, rightTitles.rotateAngle).dx;
    ...
    if (bottomTitles.showTitles && groupBarsPosition.containsAnythingToShow()) {
      ...
      for (var index = 0; index < groupBarsPosition.length; index += interval) {
        ...
        final span = TextSpan(style: Utils().getThemeAwareTextStyle(context, bottomTitles.getTextStyles(context, xValue)), text: text);
        final tp = TextPainter(text: span, textAlign: bottomTitles.textAlign, textDirection: bottomTitles.textDirection, textScaleFactor: holder.textScale);
        tp.layout();
        var x = groupBarPos.groupX;
        var y = drawSize.height + getTopOffsetDrawSize(holder) + bottomTitles.margin;
        x -= tp.width / 2;
        y -= Utils().calculateRotationOffset(tp.size, bottomTitles.rotateAngle).dy;
  ...
  void drawTouchTooltip(
    ...
    final formattedTooltipItem = formatter.format(double.parse(tooltipItem.text)).replaceAll(',', ' ');

    final span = TextSpan(
      style: Utils().getThemeAwareTextStyle(context, tooltipItem.textStyle).copyWith(color: Colors.white),
      text: formattedTooltipItem ?? tooltipItem.text,
      children: tooltipItem.children,
    );

    final tp = TextPainter(text: span, textAlign: tooltipItem.textAlign, textDirection: tooltipItem.textDirection, textScaleFactor: holder.textScale);
    ...
    final drawTooltipOnTop = tooltipData.direction == TooltipDirection.top || (tooltipData.direction == TooltipDirection.auto && showOnRodData.y >= 0);
    ...
    final roundedRect = RRect.fromRectAndCorners(rect, topLeft: radius, topRight: radius, bottomLeft: radius, bottomRight: radius);
  ...
  @visibleForTesting
  void drawStackItemBorderStroke(CanvasWrapper canvasWrapper, BarChartRodStackItem stackItem, int index, int rodStacksSize, double barThickSize, RRect barRRect, Size drawSize,
      PaintHolder<BarChartData> holder) {
    ...
    if (index == 0) {
      ...
      strokeBarRect = RRect.fromLTRBR(barRRect.left, getPixelY(stackItem.toY, drawSize, holder), barRRect.right, getPixelY(stackItem.fromY, drawSize, holder), Radius.zero);
    }
  ...
  BarTouchedSpot? handleTouch(
    ...
    if (_groupBarsPosition == null) {
      final groupsX = calculateGroupsX(viewSize, data.barGroups, data.alignment, holder);
    ...
    for (var i = 0; i < _groupBarsPosition!.length; i++) {
      ...
      for (var j = 0; j < groupBarPos.barsX.length; j++) {
        ...
        final backDrawBarY = getPixelY(targetData.barGroups[i].barRods[j].backDrawRodData.y, chartViewSize, holder);
        final touchExtraThreshold = targetData.barTouchData.touchExtraThreshold;

        final isXInTouchBounds = (touchedPoint.dx <= barX + halfBarWidth + touchExtraThreshold.right) && (touchedPoint.dx >= barX - halfBarWidth - touchExtraThreshold.left);

        final isYInBarBounds = (touchedPoint.dy <= barBotY + touchExtraThreshold.bottom) && (touchedPoint.dy >= barTopY - touchExtraThreshold.top);
